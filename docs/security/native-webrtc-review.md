...
- Runtime consumer-install hardening reviewed: CLI WebRTC helpers load `@roamhq/wrtc` lazily after validating package metadata, and fail closed unless the resolved package graph matches `@roamhq/wrtc@0.10.0`, the current platform's reviewed `@roamhq/wrtc-*` prebuilt, and `domexception@4.0.0`.
