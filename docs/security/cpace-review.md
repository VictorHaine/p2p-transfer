...
- Dependabot must keep `@cipherman/pake-js` in the `critical-pake-dependency` production group and excluded from the bulk production dependency group.
...
- `package.json`, `pnpm-lock.yaml`, or installed package metadata no longer agree on `@cipherman/pake-js@0.1.1` without an updated review.
