...
const WEBSOCKET_URL = /\b(?:ws|wss):\/\/[^\s'"`<>]+/gi;
...
export function redactCliErrorEvidence(message: string, cwd = process.cwd()): string {
  return redactLocalPathEvidence(message, cwd).replace(WEBSOCKET_URL, "[endpoint]");
}
