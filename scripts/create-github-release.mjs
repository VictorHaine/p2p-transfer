...
async function assertRemoteReleaseAssetsMatch(token, repository, releaseId, assets) {
  ...
  try {
    ...
    if (/^GitHub API /.test(errorMessage(error) ?? "")) throw error;
...
async function githubFetchJson(url, options) {
  ...
  try {
    ...
    if (error instanceof GitHubApiError) throw error;
    if (errorName(error) === "AbortError") throw new Error("GitHub API request timed out.");
    if (/^GitHub API response /.test(errorMessage(error) ?? "")) throw error;
...
function githubReleaseErrorMessage(error) {
  ...
  if (containsSensitiveErrorText(message)) return "GitHub Release creation failed with path-sensitive evidence.";
  return message;
}
