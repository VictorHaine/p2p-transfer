...
`pnpm test:browser` verifies browser sender to CLI receiver, CLI sender to browser download receiver, CLI sender to browser folder-only receiver, browser resume-key replacement, and browser folder restart after a corrupted saved partial with Playwright. It requires Chromium; set `PLAYWRIGHT_CHROMIUM=/path/to/chromium` if auto-detection fails.
