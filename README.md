...
For normal releases, update local `main` to the exact current `origin/main` commit after the protected pull request has merged, then run release preflight from that checked-out commit. Local preflight refuses unsigned `HEAD` before package or network work, and it refuses to pass if that local `HEAD` differs from GitHub's current `main` branch response.
