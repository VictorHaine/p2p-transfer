...
`pnpm test:browser` verifies browser sender to CLI receiver, CLI sender to browser download receiver, and CLI sender to browser folder-only receiver with Playwright. It requires Chromium; set `PLAYWRIGHT_CHROMIUM=/path/to/chromium` if auto-detection fails.
