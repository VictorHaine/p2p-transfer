...
The release workflow is tag-only. Release artifacts, npm publishes, and GitHub Releases are produced only from `v*.*.*` tags that match `package.json` version and exactly match protected `main`, not from manual workflow dispatches, branch-built artifacts, stale main ancestors, or off-main tag commits.
