...
test("browser receive resume is explicit and limited to saved opaque folder partials", () => {
  ...
  assert.match(webSource, /const BROWSER_RESUME_REGISTRY_DB = "ff\.browserReceiveResume\.registry\.v1";/);
  assert.match(webSource, /const BROWSER_RESUME_REGISTRY_STORE = "registry";/);
