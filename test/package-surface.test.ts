...
test("native WebRTC dependency identity and install surface stay reviewed", () => {
  ...
  assert.match(cliNativeWebrtcSource, /"darwin-arm64": \{ name: "@roamhq\/wrtc-darwin-arm64", version: "0\.10\.0", license: "BSD-2-Clause"/);
  assert.match(cliNativeWebrtcSource, /"linux-x64": \{ name: "@roamhq\/wrtc-linux-x64", version: "0\.10\.0", license: "BSD-2-Clause"/);
  assert.match(cliNativeWebrtcSource, /"win32-x64": \{ name: "@roamhq\/wrtc-win32-x64", version: "0\.10\.0", license: "BSD-2-Clause"/);
  ...
  assert.match(nativeWebrtcReview, /including reviewed wrapper metadata, optional prebuilt declarations, current-platform prebuilt metadata, domexception metadata, dependency declarations, and lifecycle-script surfaces/);
  assert.match(securityPolicy, /CLI WebRTC helpers must fail closed unless `@roamhq\/wrtc`, the current platform's `@roamhq\/wrtc-\*` prebuilt, and `domexception` match the reviewed versions, wrapper metadata, optional prebuilt declarations, current-platform prebuilt metadata, dependency declarations, lifecycle-script surfaces, and native binary location policy/);
