...
test("release workflow is tag-only, verifies one artifact, and publishes with trusted provenance", () => {
  ...
  assert.match(releaseWorkflow, /stage docker image[\s\S]*Generate staged image SBOM[\s\S]*aquasecurity\/trivy-action@a9c7b0f06e461e9d4b4d1711f154ee024b8d7ab8 # v0\.36\.0[\s\S]*format: cyclonedx[\s\S]*output: docker-image-sbom\.cdx\.json[\s\S]*name: docker-image-sbom[\s\S]*path: docker-image-sbom\.cdx\.json/);
  assert.match(releaseWorkflow, /run: node scripts\/publish-docker-image\.mjs[\s\S]*Scan staged image for vulnerabilities[\s\S]*Generate staged image SBOM[\s\S]*actions\/attest-build-provenance@a2bbfa25375fe432b6a289bc6b6cd05ecd0c4c32 # v4\.1\.0/);
