...
test("packed package smoke installs and executes published bins", () => {
  ...
  assert.match(packedSmokeScript, /const protocolVersion = requiredProtocolVersion\(parseJsonEvidence\(await readText\(path\.join\(root, "conformance", "protocol-v10\.json"\), MAX_CONFORMANCE_JSON_BYTES\), "conformance\/protocol-v10\.json"\)\.protocolVersion\)/);
  ...
  assert.match(packedSmokeScript, /process\.platform === "win32"[\s\S]*"--redact-output", "--require-private-input", "recv", "--opaque-output-names", "--code-stdin", "--yes", "--out-env", "FF_RECEIVE_OUT"/);
  ...
  assert.match(securityPolicy, /packed-install smoke must run an actual installed `ff recv` and `ff send` transfer through the installed `ff-server` using POSIX `recv --local-private-mode`, Windows explicit `--redact-output` plus `--require-private-input` plus `recv --opaque-output-names`, private stdin receive-code input, environment-sourced receive-output input backed by a `0700` output directory on POSIX, and stdin file-list input, handle child stdin pipe errors with generic non-input-reporting failures, verify the receive path is opaque, then compare received bytes/);
...
test("release workflow is tag-only, verifies one artifact, and publishes with trusted provenance", () => {
  ...
  assert.match(releaseWorkflow, /stage docker image[\s\S]*Generate staged image SBOM[\s\S]*aquasecurity\/trivy-action@a9c7b0f06e461e9d4b4d1711f154ee024b8d7ab8 # v0\.36\.0[\s\S]*format: cyclonedx[\s\S]*output: docker-image-sbom\.cdx\.json[\s\S]*actions\/attest@59d89421af93a897026c735860bf21b6eb4f7b26 # v4\.1\.0[\s\S]*sbom-path: docker-image-sbom\.cdx\.json[\s\S]*push-to-registry: true[\s\S]*name: docker-image-sbom[\s\S]*path: docker-image-sbom\.cdx\.json/);
  ...
  assert.match(githubReleaseScript, /if \(existing\.state === "published"\) \{[\s\S]*await assertRemoteReleaseAssetsMatch\(token, repository, existing\.id, assets\);[\s\S]*return \{ alreadyPublished: true \};[\s\S]*\}/);
  ...
  assert.match(releaseWorkflow, /stage docker image[\s\S]*environment: npm[\s\S]*run: node scripts\/publish-docker-image\.mjs/);
  assert.match(releaseWorkflow, /promote docker image[\s\S]*environment: npm[\s\S]*run: node scripts\/publish-docker-image\.mjs --promote/);
...
test("critical PAKE dependency identity and install surface stay reviewed", () => {
  ...
  assert.match(cpaceReview, /`@cipherman\/pake-js\/cpace` ESM subpath/);
  assert.match(cpaceReview, /`ristretto255\.init`/);
  assert.match(cpaceReview, /`ristretto255\.deriveIskInitiatorResponder`/);
  assert.match(cpaceReview, /untrusted-signaling trust boundary/);
  assert.match(cpaceReview, /Package exports reviewed: `.`, `\.\/spake2plus`, and `\.\/cpace`; this project depends on the `\.\/cpace` ESM export/);
  ...
  assert.match(cpaceReview, /including reviewed package metadata, dependency declarations, consumer lifecycle-hook policy, CPace\/curve\/hash import surfaces, and exact ESM runtime-file SHA-256 evidence/);
  ...
  assert.match(cpaceReview, /the `@cipherman\/pake-js\/cpace` ESM entry `@cipherman\/pake-js\/dist\/cpace\/index\.js`/);
  assert.match(cpaceReview, /CPace-resolved `@noble\/curves` ESM runtime files including `esm\/ed25519\.js`, `esm\/abstract\/\*\.js`, `esm\/_shortw_utils\.js`, and `esm\/utils\.js`/);
  assert.match(cpaceReview, /CPace-resolved `@noble\/hashes@1\.8\.0` ESM runtime files including `esm\/sha2\.js`, `esm\/_md\.js`, `esm\/_u64\.js`, `esm\/cryptoNode\.js`, and `esm\/utils\.js`/);
  ...
  assert.match(securityPolicy, /CLI crypto dependency runtime attestation must verify the exact resolved ESM entry file path and SHA-256 digest/);
  ...
  assert.match(cliCryptoDependenciesSource, /"esm\/ed25519\.js": "9d1ec0fa0ad8e6e8d097e4231dcd4e874975edf4410cfe6b4340dde371852b84"/);
  assert.match(cliCryptoDependenciesSource, /"esm\/abstract\/edwards\.js": "94004e8dc1a805c1d27f0d14af4c9655cdfd43cb9732b6404ba0211fa731c046"/);
  assert.match(cliCryptoDependenciesSource, /"esm\/cryptoNode\.js": "26f80c6a85b6ef7bfd83b99d95b2b5ba8764397b9465feff32e81ba4e6416619"/);
  assert.match(cliCryptoDependenciesSource, /"esm\/sha2\.js": "e729088b82e5450bff54c3a0013582aa42e1fe8f58dd31f5967f6ebe34c52299"/);
  ...
  assert.doesNotMatch(cliCryptoDependenciesSource, /"dist\/index\.cjs"|"dist\/index\.js"|"dist\/cpace\/index\.cjs"|"ed25519\.js": "33df162c066fcaef63f82118d296dcbb49ab94dc729e76c9dc5dea67f6f1da09"|"p256\.js"|"nist\.js"/);
  ...
  assert.match(cliCryptoDependenciesSource, /import\.meta\.resolve\("@cipherman\/pake-js\/cpace"\)/);
  assert.doesNotMatch(cliCryptoDependenciesSource, /requireFromCli\.resolve\("@cipherman\/pake-js"\)/);
