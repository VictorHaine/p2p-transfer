...
const RELEASE_TAG_OBJECT_SHA = "2222222222222222222222222222222222222222";
...
function verifiedReleaseTagRef() {
  return { ref: "refs/tags/v0.1.0", object: { type: "tag", sha: RELEASE_TAG_OBJECT_SHA } };
}

function verifiedReleaseTagObject(sha = RELEASE_TEST_SHA) {
  return { object: { type: "commit", sha }, verification: { verified: true, reason: "valid" } };
}

function isVerifiedReleaseTagObjectRequest(parsed: URL, method: string): boolean {
  return parsed.origin === "https://api.github.com" && method === "GET" && parsed.pathname === `/repos/VictorHaine/p2p-transfer/git/tags/${RELEASE_TAG_OBJECT_SHA}`;
}

function releaseTagObjectRequest(): string {
  return `GET https://api.github.com/repos/VictorHaine/p2p-transfer/git/tags/${RELEASE_TAG_OBJECT_SHA}`;
}
...
test("live release ref verifier rejects wrong repository before GitHub API work", async () => {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "ff-live-ref-wrong-repo-"));
  const mock = path.join(tmp, "mock-live-ref-wrong-repo.mjs");
  const log = path.join(tmp, "requests.log");
  try {
    await fs.writeFile(
      mock,
...
const log = process.env.FF_MOCK_LIVE_REF_WRONG_REPO_LOG;
...
`,
    ...
    const result = runScriptWithNodeArgs(
      ...
      {
        FF_MOCK_LIVE_REF_WRONG_REPO_LOG: log,
        GITHUB_REPOSITORY: "Attacker/p2p-transfer",
    ...
    assert.doesNotMatch(result.stderr, /Attacker|token-that-must-not-be-printed|api\.github|unexpected network|Error:/);
  ...
  try {
    globalThis.fetch = async (url, init = {}) => {
      ...
      if (parsed.origin === "https://api.github.com" && method === "GET" && parsed.pathname === "/repos/VictorHaine/p2p-transfer/git/ref/tags/v0.1.0") {
        return json(200, verifiedReleaseTagRef());
      }
      if (isVerifiedReleaseTagObjectRequest(parsed, method)) {
        return json(200, verifiedReleaseTagObject());
      }
      ...
      if (parsed.origin === "https://api.github.com" && method === "GET" && parsed.pathname === "/repos/VictorHaine/p2p-transfer/releases/99/assets") {
        assert.equal(parsed.searchParams.get("per_page"), "100");
        return json(200, releaseAssetRecords(assets));
      }
      ...
      if (parsed.origin === "https://api.github.com" && method === "GET" && parsed.pathname === "/repos/VictorHaine/p2p-transfer/releases/99") {
        return json(200, { id: 99, tag_name: "v0.1.0", name: "v0.1.0", body: "scoped release notes", draft: false, prerelease: false });
      }
      return json(500, {});
    };
    ...
    assert.deepEqual(requests.map((request) => `${request.method} ${request.url}`), [
      ...
      "GET https://api.github.com/repos/VictorHaine/p2p-transfer/releases/99/assets?per_page=100",
    ...
    assert.deepEqual(liveChecks, ["before:2", "before:10"]);
...
test("GitHub release API rejects lightweight and unverified release tags before release mutation", async () => {
  const { createGitHubRelease } = await import(`../scripts/create-github-release.mjs?release-api-tag-signature=${Date.now()}`);
  const originalFetch = globalThis.fetch;
  const requests: string[] = [];
  try {
    const run = async (mode: "lightweight" | "unverified") => {
      requests.length = 0;
      globalThis.fetch = async (url, init = {}) => {
        const parsed = new URL(String(url));
        const method = init.method ?? "GET";
        ...
        if (parsed.origin === "https://api.github.com" && method === "GET" && parsed.pathname === "/repos/VictorHaine/p2p-transfer/git/ref/tags/v0.1.0") {
          return json(
            200,
            mode === "lightweight"
              ? { ref: "refs/tags/v0.1.0", object: { type: "commit", sha: RELEASE_TEST_SHA } }
              : verifiedReleaseTagRef()
          );
        }
        if (isVerifiedReleaseTagObjectRequest(parsed, method)) {
          return json(200, {
            object: { type: "commit", sha: RELEASE_TEST_SHA },
            verification: { verified: false, reason: "bad_email", payload: "raw signature body token-that-must-not-be-printed" }
          });
        }
        return json(500, { message: "unexpected release mutation token-that-must-not-be-printed" });
      };
      await assert.rejects(
        createGitHubRelease("token-that-must-not-be-printed", "VictorHaine/p2p-transfer", "v0.1.0", RELEASE_TEST_SHA, "scoped release notes", releaseAssets()),
        (error) => {
          assert.ok(error instanceof Error);
          assert.equal(error.message, mode === "lightweight" ? "GitHub release tag must be an annotated tag." : "GitHub release tag signature was not verified.");
          assert.doesNotMatch(error.message, /token-that-must-not-be-printed|raw signature body|bad_email|unexpected release mutation/);
          return true;
        }
      );
      assert.deepEqual(
        ...
        mode === "lightweight"
          ? ["GET https://api.github.com/repos/VictorHaine/p2p-transfer/git/ref/tags/v0.1.0"]
          : ["GET https://api.github.com/repos/VictorHaine/p2p-transfer/git/ref/tags/v0.1.0", releaseTagObjectRequest()]
      );
    };

    await run("lightweight");
    await run("unverified");
  } finally {
    globalThis.fetch = originalFetch;
  }
});
...
test("GitHub release API recovers an existing draft release on rerun", async () => {
  ...
  try {
    ...
    assert.deepEqual(liveChecks, ["before:2", "before:5", "before:13"]);
...
test("GitHub release API accepts an existing published release on rerun when assets match", async () => {
  ...
  try {
    globalThis.fetch = async (url, init = {}) => {
      ...
      if (parsed.origin === "https://api.github.com" && method === "GET" && parsed.pathname === "/repos/VictorHaine/p2p-transfer/releases/tags/v0.1.0") {
        return json(200, { id: 88, tag_name: "v0.1.0", name: "v0.1.0", body: "scoped release notes", draft: false, prerelease: false });
      }
    ...
    assert.deepEqual(liveChecks, ["before:2"]);
    assert.deepEqual(requests, [
      "GET https://api.github.com/repos/VictorHaine/p2p-transfer/git/ref/tags/v0.1.0",
      releaseTagObjectRequest(),
      "POST https://api.github.com/repos/VictorHaine/p2p-transfer/releases",
...
test("GitHub release API rejects existing published releases whose metadata does not match", async () => {
  const { createGitHubRelease } = await import(`../scripts/create-github-release.mjs?release-rerun-published-metadata=${Date.now()}`);
  const originalFetch = globalThis.fetch;
  const requests: string[] = [];
  try {
    globalThis.fetch = async (url, init = {}) => {
      ...
      if (parsed.origin === "https://api.github.com" && method === "GET" && parsed.pathname === "/repos/VictorHaine/p2p-transfer/releases/tags/v0.1.0") {
        return json(200, { id: 88, tag_name: "v0.1.0", name: "v0.1.0", body: "tampered notes token-that-must-not-be-printed", draft: false, prerelease: false });
      }
      return json(500, { message: "unexpected route token-that-must-not-be-printed" });
    };
    ...
    await assert.rejects(
      ...
      (error) => {
        assert.ok(error instanceof Error);
        assert.equal(error.message, "existing published GitHub release metadata does not match verified release metadata.");
        assert.doesNotMatch(error.message, /token-that-must-not-be-printed|tampered|private/);
        return true;
      }
    );
    assert.deepEqual(requests, [
    ...
    assert.equal(requests.some((request) => /PATCH|DELETE|uploads\.github\.com|\/assets/.test(request)), false);
  } finally {
    globalThis.fetch = originalFetch;
  }
});
...
test("GitHub release API rejects tags that no longer point at the workflow commit", async () => {
  ...
  try {
    globalThis.fetch = async (url, init = {}) => {
      ...
      if (isVerifiedReleaseTagObjectRequest(parsed, method)) {
        return json(200, verifiedReleaseTagObject("fedcba9876543210fedcba9876543210fedcba98"));
      }
      return json(500, {});
    };
    ...
    assert.deepEqual(requests, ["GET https://api.github.com/repos/VictorHaine/p2p-transfer/git/ref/tags/v0.1.0", releaseTagObjectRequest()]);
  } finally {
    globalThis.fetch = originalFetch;
  }
});
...
test("GitHub release API rejects successful final publish when published metadata does not match", async () => {
  const { createGitHubRelease } = await import(`../scripts/create-github-release.mjs?release-publish-success-metadata-mismatch=${Date.now()}`);
  const originalFetch = globalThis.fetch;
  const requests: string[] = [];
  const assets = releaseAssets();
  try {
    globalThis.fetch = async (url, init = {}) => {
      ...
      if (parsed.origin === "https://api.github.com" && method === "GET" && parsed.pathname === "/repos/VictorHaine/p2p-transfer/releases/99") {
        return json(200, { id: 99, tag_name: "v0.1.0", name: "v0.1.0", body: "tampered notes token-that-must-not-be-printed", draft: false, prerelease: false });
      }
      return json(500, { message: "unexpected route token-that-must-not-be-printed" });
    };
    ...
    await assert.rejects(
      ...
      (error) => {
        assert.ok(error instanceof Error);
        assert.equal(error.message, "existing published GitHub release metadata does not match verified release metadata.");
        assert.doesNotMatch(error.message, /token-that-must-not-be-printed|tampered/);
        return true;
      }
    );
    assert.deepEqual(requests, [
...
test("GitHub release API rejects final publish ambiguity when published metadata does not match", async () => {
  const { createGitHubRelease } = await import(`../scripts/create-github-release.mjs?release-publish-metadata-mismatch=${Date.now()}`);
  const originalFetch = globalThis.fetch;
  const requests: string[] = [];
  const assets = releaseAssets();
  try {
    globalThis.fetch = async (url, init = {}) => {
      ...
      if (parsed.origin === "https://api.github.com" && method === "PATCH" && parsed.pathname === "/repos/VictorHaine/p2p-transfer/releases/99") {
        return json(500, { message: "ambiguous publish failure token-that-must-not-be-printed" });
      }
      if (parsed.origin === "https://api.github.com" && method === "GET" && parsed.pathname === "/repos/VictorHaine/p2p-transfer/releases/99") {
        return json(200, { id: 99, tag_name: "v0.1.0", name: "v0.1.0", body: "tampered notes token-that-must-not-be-printed", draft: false, prerelease: false });
      }
    ...
    await assert.rejects(
      ...
      (error) => {
        assert.ok(error instanceof Error);
        assert.equal(error.message, "GitHub API request failed with HTTP status 500.");
        assert.doesNotMatch(error.message, /token-that-must-not-be-printed|tampered|ambiguous/);
        return true;
      }
    );
    assert.deepEqual(requests, [
    ...
    assert.equal(requests.some((request) => request.startsWith("DELETE ")), false);
    assert.equal(requests.filter((request) => /\/assets\?per_page=100/.test(request)).length, 1);
  } finally {
    globalThis.fetch = originalFetch;
  }
});
...
test("Docker publish script rejects branch refs before package, smoke, or push work", () => {
  const result = runScript("scripts/publish-docker-image.mjs", {
    ...releaseTagEnv("v0.1.0"),
    GITHUB_REF_TYPE: "branch",
    GITHUB_REF: "refs/heads/v0.1.0",
  ...
  assert.equal(result.stdout, "");
  assert.match(result.stderr, /Docker image publish failed:\n- release workflow ref must be the matching tag ref\./);
  assert.doesNotMatch(result.stderr, /refs\/heads|package metadata|token-that-must-not-be-used|release docker policy smoke|docker release|api\.github|Error:/);
});

test("Docker publish script rejects non-file package metadata after release env validation and before smoke or push work", async () => {
...
test("Docker publish script rejects wrong repositories before package, smoke, or push work", async () => {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "ff-docker-publish-repository-"));
  const scripts = path.join(tmp, "scripts");
  try {
    await fs.mkdir(scripts);
    await fs.mkdir(path.join(tmp, "package.json"));
    ...
    assert.doesNotMatch(result.stderr, /Attacker|package metadata|token-that-must-not-be-used|release docker policy smoke|docker release|api\.github|Error:/);
  } finally {
    await fs.rm(tmp, { force: true, recursive: true });
  }
});
...
test("GitHub release controls reject control-bearing stdin tokens before network work", async () => {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "ff-release-controls-stdin-token-"));
  const mock = path.join(tmp, "mock-release-controls-stdin-token-fetch.mjs");
  const log = path.join(tmp, "requests.log");
  try {
    await fs.writeFile(
      mock,
...
const log = process.env.FF_MOCK_RELEASE_CONTROLS_STDIN_TOKEN_LOG;
...
`,
    ...
    const result = runScriptWithNodeArgs(
      "scripts/configure-github-release-controls.mjs",
      {
        FF_MOCK_RELEASE_CONTROLS_STDIN_TOKEN_LOG: log
      },
      ["--dry-run", "--token-stdin"],
      ["--import", mock],
      "token-that-must-not-be-used\nworkflow=true"
    ...
    assert.doesNotMatch(result.stderr, /token-that-must-not-be-used|workflow=true|unexpected network|api\.github|Error:/);
    assert.equal(requests, "");
  } finally {
    await fs.rm(tmp, { force: true, recursive: true });
  }
...
const log = process.env.FF_MOCK_RELEASE_CONTROLS_STDIN_TTY_LOG;
...
`,
    ...
    const result = runScriptWithNodeArgs(
      "scripts/configure-github-release-controls.mjs",
      {
        FF_MOCK_RELEASE_CONTROLS_STDIN_TTY_LOG: log
      },
      ["--dry-run", "--token-stdin"],
      ["--import", mock],
      "token-that-must-not-be-used"
    ...
    assert.notEqual(result.status, 0);
    assert.equal(result.stdout, "");
    assert.match(result.stderr, /GitHub release control setup failed:\n- Pipe GitHub token stdin; interactive terminal stdin is not accepted for --token-stdin\./);
    assert.doesNotMatch(result.stderr, /token-that-must-not-be-used|unexpected network|api\.github|Error:/);
    assert.equal(requests, "");
  } finally {
    await fs.rm(tmp, { force: true, recursive: true });
  }
...
test("GitHub release controls reject ambiguous stdin and environment tokens before network work", async () => {
...
globalThis.fetch = async (url, init = {}) => {
  ...
  if (method === "GET" && path === "/repos/VictorHaine/p2p-transfer") return json(200, {
    id: 1,
    security_and_analysis: repositorySecurityEnabled ? ${JSON.stringify(ENABLED_SECURITY_AND_ANALYSIS)} : {
  ...
  if (method === "PATCH" && path === "/repos/VictorHaine/p2p-transfer") {
    if (body?.security_and_analysis?.secret_scanning?.status !== "enabled" || body?.security_and_analysis?.secret_scanning_push_protection?.status !== "enabled") return json(400, {});
    return json(200, {});
  }
  if (method === "PUT" && path === "/repos/VictorHaine/p2p-transfer/automated-security-fixes") {
...
`,
    ...
    assert.deepEqual(repositoryPatch.body, {
...
test("GitHub release controls verify repository security before mutating environments", async () => {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "ff-release-controls-repository-security-"));
  const mock = path.join(tmp, "mock-github-fetch.mjs");
  const log = path.join(tmp, "requests.log");
  try {
    await fs.writeFile(
...
`,
    ...
    assert.notEqual(result.status, 0);
    assert.equal(result.stdout, "");
    assert.match(result.stderr, /Release readiness check failed:\n- Set GITHUB_TOKEN or GH_TOKEN, or pipe a token with --token-stdin, before running release preflight\./);
...
test("release preflight rejects control-bearing stdin GitHub tokens before package or network work", async () => {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "ff-release-preflight-stdin-token-"));
  const mock = path.join(tmp, "mock-release-preflight-stdin-token-fetch.mjs");
  const log = path.join(tmp, "requests.log");
  try {
    await fs.writeFile(
      mock,
...
const log = process.env.FF_MOCK_PREFLIGHT_STDIN_TOKEN_LOG;
...
`,
    ...
    const result = runScriptWithNodeArgs(
      "scripts/check-release-readiness.mjs",
      {
        FF_MOCK_PREFLIGHT_STDIN_TOKEN_LOG: log
      },
      ["--token-stdin"],
      ["--import", mock],
      "token-that-must-not-be-used\nworkflow=true"
...
test("release preflight rejects interactive stdin GitHub token input before package or network work", async () => {
  const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "ff-release-preflight-stdin-tty-"));
  const mock = path.join(tmp, "mock-release-preflight-stdin-tty.mjs");
  const log = path.join(tmp, "requests.log");
  try {
    await fs.writeFile(
      mock,
...
const log = process.env.FF_MOCK_PREFLIGHT_STDIN_TTY_LOG;
...
`,
    ...
    const result = runScriptWithNodeArgs(
      "scripts/check-release-readiness.mjs",
      {
        FF_MOCK_PREFLIGHT_STDIN_TTY_LOG: log
      },
      ["--token-stdin"],
      ["--import", mock],
      "token-that-must-not-be-used"
...
test("release preflight rejects ambiguous stdin and environment GitHub tokens before package or network work", async () => {
...
globalThis.fetch = async (url, init = {}) => {
  ...
  if (method === "GET" && path === "/repos/VictorHaine/p2p-transfer/actions/workflows/scorecard.yml/runs?branch=main&per_page=20") {
    return json(200, { workflow_runs: [{ status: "completed", conclusion: "success", head_branch: "main", head_sha: mainSha }] });
  }
  if (method === "GET" && path === "/repos/VictorHaine/p2p-transfer/actions/workflows/dependency-integrity.yml/runs?branch=main&per_page=20") {
    return json(200, { workflow_runs: [{ status: "completed", conclusion: "success", head_branch: "main", head_sha: mainSha }] });
  }
...
const codeqlRunNewerPending = process.env.FF_MOCK_CODEQL_NEWER_PENDING === "true";
...
`,
    ...
    assert.match(requests, /GET https:\/\/api\.github\.com\/repos\/VictorHaine\/p2p-transfer\/actions\/workflows\/codeql\.yml\/runs\?branch=main&per_page=20\n/);
    assert.match(requests, /GET https:\/\/api\.github\.com\/repos\/VictorHaine\/p2p-transfer\/actions\/workflows\/scorecard\.yml\/runs\?branch=main&per_page=20\n/);
    assert.match(requests, /GET https:\/\/api\.github\.com\/repos\/VictorHaine\/p2p-transfer\/actions\/workflows\/dependency-integrity\.yml\/runs\?branch=main&per_page=20\n/);
    ...
    assert.match(staleCodeqlResult.stderr, /GitHub codeql workflow has no successful current-main run in the recent run history\./);
    ...
    assert.match(failedCodeqlResult.stderr, /GitHub codeql workflow has no successful current-main run in the recent run history\./);
    assert.doesNotMatch(failedCodeqlResult.stderr, /token-that-must-not-be-printed|failure|api\.github|registry\.npmjs|Error:/);

    const newerPendingCodeqlResult = runScriptWithNodeArgs(
      "scripts/check-release-readiness.mjs",
      {
        FF_MOCK_PREFLIGHT_LOG: log,
        FF_MOCK_CODEQL_NEWER_PENDING: "true",
        GITHUB_ACTIONS: "true",
        GITHUB_ACTOR: "tagger",
        GITHUB_TOKEN: "ghs_token-that-must-not-be-printed"
      },
    ...
    assert.equal(newerPendingCodeqlResult.status, 0, newerPendingCodeqlResult.stderr);
    assert.equal(newerPendingCodeqlResult.stderr, "");
...
function releaseAssetRecords(assets: Array<{ name: string; bytes: Buffer }>) {
