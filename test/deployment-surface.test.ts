...
const PINNED_ACTIONS = new Map([
  ...
  ["actions/attest", { sha: "59d89421af93a897026c735860bf21b6eb4f7b26", version: "v4.1.0" }],
...
test("CI and release workflows keep minimal token permissions", () => {
  ...
  assert.match(releaseDockerStageJob, /Scan staged image for vulnerabilities[\s\S]*aquasecurity\/trivy-action@a9c7b0f06e461e9d4b4d1711f154ee024b8d7ab8 # v0\.36\.0[\s\S]*image-ref: \$\{\{ steps\.docker_image\.outputs\.image \}\}@\$\{\{ steps\.docker_image\.outputs\.digest \}\}[\s\S]*exit-code: "1"[\s\S]*vuln-type: os,library[\s\S]*severity: UNKNOWN,LOW,MEDIUM,HIGH,CRITICAL[\s\S]*TRIVY_USERNAME: \$\{\{ github\.actor \}\}[\s\S]*TRIVY_PASSWORD: \$\{\{ github\.token \}\}/);
...
test("checked GitHub release controls setup matches the protected release surface", () => {
  ...
  assert.match(githubReleaseControlsScript, /assertDependabotAutomatedSecurityFixes\(await github\(token, "GET", `\/repos\/\$\{repository\}\/automated-security-fixes`\)\)/);
  ...
  assert.match(githubReleaseControlsScript, /GitHub repository Dependabot security updates must not be paused\./);
  ...
  assert.match(securityPolicy, /reject wrong-repository or non-Actions release contexts before token use, package reads, npm registry requests, Docker login\/push, GitHub API requests, or repository mutation/);
...
test("release preflight checks external GitHub release prerequisites", () => {
  ...
  assert.match(readme, /checked release-control setup enables and re-reads those repository security controls, verifies the dedicated Dependabot status is unpaused/);
  assert.match(readme, /release preflight reads GitHub `security_and_analysis`, `vulnerability-alerts`, and `automated-security-fixes` endpoints so tagging fails if any feature is disabled, paused, or hidden from the release token/);
  assert.match(readme, /read repository metadata including `security_and_analysis`, dependency vulnerability alert status, private vulnerability reporting status, the `main` branch, Actions secret metadata, Actions workflow run metadata, repository rulesets including bypass actors, repository environments, and deployment branch policies/);
  ...
  assert.match(securityPolicy, /must allow an already-existing target npm version only when an explicit rerun flag is present inside the matching GitHub release tag workflow context/);
  assert.match(securityPolicy, /must still verify the npm package exists, any bootstrap placeholder is not `latest`, private vulnerability reporting is enabled, dependency vulnerability alerts are enabled, repository secret scanning, push protection, and Dependabot security updates are enabled and unpaused/);
  ...
  assert.match(releaseReadinessScript, /Existing npm versions may only be allowed during a matching GitHub release tag workflow rerun\./);
  ...
  assert.match(releaseReadinessScript, /async function collectGitHubRepositoryReadiness\(failures, token, repository, authenticatedLogin, releaseActorLogin, localHeadSha\)/);
...
test("trusted reverse-proxy client IP handling is explicit and documented", () => {
  ...
  assert.match(configSource, /TRUSTED_PROXY_IPS CIDR ranges must not trust every address/);
