...
async function createFixture() {
  ...
  await fs.writeFile(path.join(artifactDir, "SBOM.cdx.json"), Buffer.from('{"bomFormat":"CycloneDX"}\n', "utf8"));
  return { artifactDir, root, script: path.join(scriptsDir, "write-release-checksum.mjs") };
}

test("release checksum writer emits checksums for the exact packed tarball and SBOM", async () => {
...
test("release checksum writer rejects artifact directory widening without path leakage", async () => {
  ...
  try {
    ...
    assert.equal(result.stdout, "");
    assert.match(result.stderr, /Release checksum generation failed:\nrelease artifact directory must contain exactly the expected tarball and SBOM\./);
